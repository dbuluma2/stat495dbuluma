...
Verify that the Binomial distribution is an exponential family. Note, $X \sim Bin(n,p)$ has pmf given by:
...
The data was pre-processed. The text states that the predictors were standardized to mean 0 and sum of squares 1, and the response had it's mean subtracted off (i.e. it was centered), but not scaled. So, we do that just to match (clumsily).
...
(b) Perform an regression using an automated variable selection algorithm of your choice and choose a model based on an appropriate descriptive statistic. Compute the test MSE.
